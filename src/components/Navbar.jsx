          ...
          <div className="hidden md:flex items-center space-x-8">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-zinc-300 hover:text-red-600 transition font-medium"
              >
                {link.name}
              </a>
            ))}
          ...
          <div className="px-2 pt-2 pb-3 space-y-1">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="block px-3 py-2 text-zinc-300 hover:text-red-600 hover:bg-zinc-800 rounded transition"
